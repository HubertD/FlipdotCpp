...
void TetrisBlock::draw(FlipdotGfx& gfx, int offsetX, int offsetY, int pointSizeX, int pointSizeY)
{
	...
	for (int iy=0; iy<4; iy++)
	{
		for (int ix=0; ix<4; ix++)
		{
			if (getPoint(ix, iy))
			{
				gfx.drawRect(px+pointSizeX*ix, py+pointSizeY*iy, pointSizeX, pointSizeY, true);
			}
		}
	}
